        ...
        <el-card class="box-card">
            ...
            <el-table :row-style="{ height: '55px' }" id="out-table" :data="list" style="width: 100%;height: 700px;"
                ...
                <el-table-column prop="status" label="当前" min-width="100" align="center">
                    <template slot-scope="scope">
                        <el-tag style="height:25px; line-height:25px;"
                            :type="scope.row.state == '0' ? 'warning' : scope.row.state == '1' ? 'success' : scope.row.state == '2' ? 'primary' : scope.row.state == '3' ? 'info' : 'danger'"
                            disable-transitions>{{
                                scope.row.status == '0' ? "未出生" : scope.row.state == '1' ? "在世" : scope.row.state == '2' ? "在地府"
                                    : scope.row.state == '3' ? "轮回中" : "受刑中"
                            }}
                        </el-tag>
                    </template>
                </el-table-column>
